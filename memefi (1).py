    ...
    for attempt in range(retries):
        async with session.post(url, headers=headers, json=json_payload) as response:
            if response.status == 200:
                ...
                if attempt < retries - 1:  # Bu son deneme değilse, tekrar denemeden önce bekleyin.
                    await asyncio.sleep(10)
                else:
                    print("❌ Birkaç deneme başarısız oldu. Yeniden başlatılıyor....")
                    return None
    return None
...
async def fetch(account_line):
    ...
    url = 'https://api-gw-tg.memefi.club/graphql'
    headers = headers_set.copy()  # headers_set'in bir kopyasını oluşturarak global değişkeni değiştirmemesini sağlıyorum
    data = {
...
async def cek_user(index):
    ...
    headers = headers_set.copy()  # Global değişkeni değiştirmemek için headers_set'in bir kopyasını oluşturuyorum
        ...
        async with session.post(url, headers=headers, json=json_payload) as response:
            if response.status == 200:
                ...
                if 'errors' in response_data:
                    print(f"❌ Sorgu kimliği yanlış")
                    return None
                else:
                    user_data = response_data['data']['telegramUserMe']
                    return user_data  # Yanıt sonucunu geri döndürüyorum
            else:
                print(response)
                print(f"❌ Gagal dengan status {response.status}, mencoba lagi...")
                return None  # Hata oluşursa None değerini döndürüyorum
